...
def windowed_dataset(series, window_size, batch_size, shuffle_buffer):
    series = tf.expand_dims(series, axis=-1)                                # (n, n) 을 (n, n, 1) 로 바꿔준다
    print(series)
    ds = tf.data.Dataset.from_tensor_slices(series)                         # 데이터셋화...?
    ds = ds.window(window_size + 1, shift=1, drop_remainder=True)           # 시계열 데이터 생성, 31개의 연속된 데이터, 1칸씩
    ds = ds.flat_map(lambda w: w.batch(window_size + 1))                    # (31,)
    ds = ds.shuffle(shuffle_buffer)                                         # 셔플
    ds = ds.map(lambda w: (w[:-1], w[1:]))                                  # x, y 값을 나눈다
    return ds.batch(batch_size).prefetch(1)                                 # 일종의 데이터 제너레이터...?
