...
# Flatten
L_flat = tf.reshape(L4, [-1, 7*7*32])
print(L_flat) # Tensor("Reshape:0", shape=(?, 256), dtype=float32)

# L5.
w5 = tf.compat.v1.get_variable('w5', shape = [7*7*32, 64], initializer = tf.keras.initializers.HeNormal())
...
optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate = 1*1e-4)
