...
x = []
for i in quantiles:
    filepath_cp = f'../dacon/data/modelcheckpoint/dacon_standard_y1_quantile_{i:.1f}.hdf5'
...
# 모레!!
for i in quantiles:
    filepath_cp = f'../dacon/data/modelcheckpoint/dacon_standard_y2_quantile_{i:.1f}.hdf5'
    model = load_model(filepath_cp, compile = False)
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
    pred = pd.DataFrame(model.predict(x_test).round(2))
    x.append(pred)
...
# submission.to_csv('./practice/dacon/data/0120_trystandard.csv', index = False)
